(() => {
  ...
  /* ---------------------------------------------------------
     3. Keyboard-driven menu (T / O / S / Q)
  --------------------------------------------------------- */
  const items = Array.from(document.querySelectorAll('.menu-item'));
  const byKey = {};
  items.forEach((item) => { byKey[item.dataset.key] = item; });

  function pulse(item) {
    item.classList.add('active');
    setTimeout(() => item.classList.remove('active'), 550);
  }

  window.addEventListener('keydown', (e) => {
    const k = e.key.toUpperCase();
    if (byKey[k]) pulse(byKey[k]);
  });

  items.forEach((item) => {
    item.addEventListener('click', () => pulse(item));
  });

  /* ---------------------------------------------------------
     4. Ambient braille heartagram sigil (canvas)
